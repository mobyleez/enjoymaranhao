const destinations = [
  {
    ...
    tag: 'Imperdível',
    sub: 'Dunas brancas e lagoas cristalinas',
    large: true,
    bg: 'linear-gradient(135deg, #1B7BB4 0%, #7AC4E2 40%, #EDEAE3 100%)',
    svg: (
  ...
  },
  {
    ...
    sub: 'A ilha capital com azulejos e história',
    large: false,
    bg: 'linear-gradient(135deg, #6B1E4E 0%, #E85C4A 50%, #F5A623 100%)',
    svg: (
  ...
  },
  {
    ...
    tag: 'Natureza',
    sub: 'Cachoeiras e formações rochosas deslumbrantes',
    large: false,
    bg: 'linear-gradient(135deg, #1A7875 0%, #2a9e6e 50%, #F5A623 100%)',
    svg: (
      <svg width="100%" height="100%" viewBox="0 0 300 260" preserveAspectRatio="xMidYMid slice" xmlns="http://www.w3.org/2000/svg">
  ...
  },
  {
    name: 'Barreirinhas',
    tag: 'Ecoturismo',
    sub: 'Portal dos Lençóis Maranhenses',
    large: false,
    bg: 'linear-gradient(135deg, #F5A623 0%, #E85C4A 50%, #6B1E4E 100%)',
    svg: (
      <svg width="100%" height="100%" viewBox="0 0 300 260" preserveAspectRatio="xMidYMid slice" xmlns="http://www.w3.org/2000/svg">
  ...
  },
  {
    ...
    tag: 'Aventura',
    sub: 'Encontro do rio com o mar',
    large: false,
    bg: 'linear-gradient(135deg, #7AC4E2 0%, #1A7875 50%, #0D0D0D 100%)',
    svg: (
      <svg width="100%" height="100%" viewBox="0 0 300 260" preserveAspectRatio="xMidYMid slice" xmlns="http://www.w3.org/2000/svg">
...
const Destinations = () => {
  return (
    <section className="bg-background py-20 lg:py-[120px] px-8 lg:px-16 relative overflow-hidden" id="destinos">
      <div className="flex flex-col lg:flex-row justify-between lg:items-end mb-16 gap-4 reveal">
        <div>
          ...
          <h2 className="font-playfair font-bold leading-[1.05] tracking-[-2px]" style={{ fontSize: 'clamp(36px, 5vw, 66px)' }}>
            Onde cada paisagem<br />conta uma <em className="italic grad-text-diag">história.</em>
          </h2>
        </div>
        <a href="#" className="text-xs tracking-[2px] uppercase text-[rgba(255,255,255,0.4)] no-underline flex items-center gap-2 transition-colors duration-200 whitespace-nowrap hover:text-white cursor-none">
          Ver todos <span className="text-base transition-transform duration-200">→</span>
        </a>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-[1.6fr_1fr_1fr] lg:grid-rows-2 gap-4">
        {destinations.map((dest, i) => (
          <div
            key={i}
            className={`rounded-[20px] overflow-hidden relative cursor-none group ${
              dest.large ? 'lg:row-span-2 min-h-[360px] lg:min-h-[560px]' : 'min-h-[220px] lg:min-h-[260px]'
            }`}
          >
            <div
              className="absolute inset-0 transition-transform duration-700 ease-[cubic-bezier(0.25,0.46,0.45,0.94)] group-hover:scale-105"
              style={{ background: dest.bg, minHeight: '100%' }}
            >
              <div className="absolute inset-0">{dest.svg}</div>
            </div>
            ...
            <div className="absolute bottom-0 left-0 right-0 p-7 translate-y-1 transition-transform duration-300 group-hover:translate-y-0">
              <span className="inline-block text-[10px] tracking-[2px] uppercase py-1 px-3 rounded-full bg-[rgba(255,255,255,0.12)] backdrop-blur-[8px] mb-2.5 text-[rgba(255,255,255,0.8)] font-medium">
                {dest.tag}
              </span>
              <p className="font-playfair font-bold text-white leading-[1.15]" style={{ fontSize: 'clamp(20px, 2.5vw, 32px)' }}>
                {dest.name}
              </p>
