const Hero = () => {
  return (
    <section className="min-h-screen flex flex-col justify-end px-8 lg:px-16 pb-16 lg:pb-20 relative overflow-hidden">
      {/* Background */}
      ...
      {/* Decorative number */}
      <div className="absolute right-[-20px] top-1/2 -translate-y-1/2 font-playfair font-black text-transparent leading-none select-none pointer-events-none tracking-[-20px]" style={{
      ...
      <div className="absolute right-[15%] top-1/2 animate-float pointer-events-none hidden lg:block" style={{
      ...
      {/* Gradient line */}
      <div className="absolute left-0 right-0 h-[2px] grad-bg opacity-60 top-1/2 animate-line-reveal" />

      {/* Tag */}
      <p className="animate-fade-up-1 text-[11px] tracking-[5px] uppercase font-semibold grad-text relative z-10 mb-5">
        TURISMO · MARANHÃO · BRASIL
      </p>

      {/* Title */}
      <h1 className="animate-fade-up-2 font-playfair font-black leading-[0.92] tracking-[-3px] max-w-[60%] lg:max-w-[60%] max-lg:max-w-full mb-8 relative z-10" style={{ fontSize: 'clamp(44px, 7vw, 110px)' }}>
        O Brasil que poucos conhecem. Todos <em className="italic grad-text-diag">deveriam.</em>
      </h1>

      {/* Bottom section */}
      <div className="animate-fade-up-3 flex flex-col lg:flex-row items-start lg:items-end justify-between relative z-10 gap-8">
        <p className="text-[15px] text-[rgba(255,255,255,0.45)] max-w-[380px] leading-[1.8] font-light">
          Das dunas brancas dos Lençóis ao ritmo do Bumba Meu Boi — descubra o Maranhão com quem verdadeiramente o ama.
        </p>

        <div className="flex flex-col items-start lg:items-end gap-4">
          <a href="#destinos" className="inline-flex items-center gap-3 py-[18px] px-10 rounded-full grad-bg text-white text-[13px] tracking-[2px] uppercase font-semibold no-underline font-dm relative overflow-hidden transition-all duration-300 hover:-translate-y-[3px] shadow-[0_8px_40px_rgba(27,123,180,0.25)] hover:shadow-[0_16px_60px_rgba(27,123,180,0.4)] cursor-none">
            EXPLORAR DESTINOS
