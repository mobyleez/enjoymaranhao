...
const Packages = () => {
  return (
    <section className="bg-secondary py-16 lg:py-[120px] px-5 lg:px-16 relative" id="pacotes">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 lg:gap-6 mt-8 lg:mt-[60px]">
        {packages.map((pkg, i) => {
          const isFeatured = pkg.type === 'featured';
          return (
            <div
              key={i}
              className={`rounded-2xl lg:rounded-3xl overflow-hidden relative transition-transform duration-400 hover:-translate-y-1.5 reveal ${
            ...
            >
              ...
              <div className="p-6 lg:p-10 pb-5 lg:pb-7">
                {pkg.badge && (
                  <span className="inline-block text-[9px] tracking-[2px] uppercase py-1.5 px-3 rounded-full grad-bg text-white font-semibold mb-3 lg:mb-4">
                    {pkg.badge}
                  </span>
                )}
                <p className={`font-playfair text-xl lg:text-[26px] font-bold leading-[1.15] mb-2 whitespace-pre-line ${isFeatured ? 'text-foreground' : 'text-secondary-foreground'}`}>
                  {pkg.name}
                </p>
                <p className={`text-sm lg:text-[13px] leading-[1.7] font-light ${isFeatured ? 'text-[rgba(255,255,255,0.45)]' : 'text-[#888]'}`}>
                  {pkg.subtitle}
                </p>
              </div>

              <div className={`px-6 lg:px-10 py-4 lg:py-6 flex items-baseline gap-1.5 border-t ${isFeatured ? 'border-[rgba(255,255,255,0.06)]' : 'border-[rgba(0,0,0,0.06)]'}`}>
                <span className={`text-[10px] lg:text-[11px] tracking-[1px] uppercase ${isFeatured ? 'text-[rgba(255,255,255,0.35)]' : 'text-[#999]'}`}>
                  {pkg.priceLabel}
                </span>
                <span className={`font-playfair font-black grad-text tracking-[-1px] ${pkg.priceSmall ? 'text-2xl lg:text-[28px]' : 'text-3xl lg:text-[38px]'}`}>
                  {pkg.price}
                </span>
                {pkg.priceSuffix && (
                  <span className={`text-sm lg:text-base font-semibold ${isFeatured ? 'text-[rgba(255,255,255,0.4)]' : 'text-[#aaa]'}`}>
                    {pkg.priceSuffix}
                  </span>
                )}
              </div>
              ...
              <div className="px-6 lg:px-10 pb-6 lg:pb-10">
                {pkg.items.map((item, j) => (
                  <div
                    key={j}
                    className={`flex items-center gap-3 text-sm lg:text-[13px] py-2.5 lg:py-2 border-b ${
              ...
              <div className="px-6 lg:px-10 pb-6 lg:pb-10">
                <a
                  href="#"
                  className={`flex items-center justify-center text-center min-h-[52px] py-4 rounded-full text-xs tracking-[2px] uppercase font-semibold no-underline font-dm transition-all duration-200 ${
                    pkg.btnStyle === 'grad'
